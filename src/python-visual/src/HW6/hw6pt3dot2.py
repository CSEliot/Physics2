...
myDisplay = gdisplay()
myVXGraph = gcurve(color=color.red)
myAGraph = gcurve(color=color.white)
myVYgraph = gcurve(color=color.red)
...
'''VARIABLES'''
hoop_center = (10, 8, 0)
center_of_room = (10, 0, 0)
origin = (0, 1, 0)
deltat = 0.1
vball = vector(0.5, 0.866, 0)  # angle's unit vector
vball = vball * 15  # net force of 15 for initial velocity.
t = 0.0
ball_weight = 0.05
ball_size = 0.6
...
'''OBJECTS'''
floor = box(pos=center_of_room, length=20, height=0, width=4, color=color.white)
ball = sphere(pos=origin, radius=ball_size, color=color.green)

while ball.y >= 1:
    myVXGraph.plot(pos=(t, ball.x))
    myVYgraph.plot(pos=(t, ball.y))
